import common, crc, internal, std/os, std/strutils, std/tables,
...
type
  ...
  ZipArchiveBytesReader* = ref object
    file_bytes: string
    records: OrderedTable[string, ZipArchiveRecord]

iterator walkFiles*(reader: ZipArchiveBytesReader): string =
...
proc extractFile*(
  reader: ZipArchiveBytesReader, path: string
  ...
  let
    src = cast[ptr UncheckedArray[uint8]](reader.file_bytes[0].addr)
    record =
      try:
        reader.records[path]
      except KeyError:
  ...
  if pos + fileHeaderLen > reader.file_bytes.len:
    failArchiveEOF()
  ...
  if pos + record.compressedSize > reader.file_bytes.len:
    failArchiveEOF()
...
proc close*(reader: ZipArchiveBytesReader) {.raises: [OSError].} =
...
proc findEndOfCentralDirectory(reader: ZipArchiveBytesReader): int =
  let src = cast[ptr UncheckedArray[uint8]](reader.file_bytes[0].addr)

  result = reader.file_bytes.len - 22 # Work backwards in the file starting here
  while true:
    if result < 0:
      failArchiveEOF()
...
proc openZipArchiveBytes*(
  bytes: string
): ZipArchiveBytesReader {.raises: [IOError, OSError, ZippyError].} =
  result = ZipArchiveBytesReader()
  result.file_bytes = bytes

  try:
    let src = cast[ptr UncheckedArray[uint8]](result.file_bytes[0].addr)

    let eocd = result.findEndOfCentralDirectory()
    if eocd + 22 > result.file_bytes.len:
      failArchiveEOF()
    ...
    if zip64:
      ...
      if pos + 64 > result.file_bytes.len:
        failArchiveEOF()
    ...
    for _ in 0 ..< numCentralDirectoryRecords:
      if pos + 46 > result.file_bytes.len:
        failArchiveEOF()
      ...
      if pos + fileNameLen > result.file_bytes.len:
        failArchiveEOF()
      ...
      block: # Handle zip64 values as needed
        ...
        while extraFieldsOffset < pos + extraFieldLen:
          if pos + 4 > result.file_bytes.len:
            failArchiveEOF()
...
proc extractAll*(
  ...
  let
    reader = openZipArchiveBytes(bytes)
    src = cast[ptr UncheckedArray[uint8]](reader.file_bytes[0].addr)
